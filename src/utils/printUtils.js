 ...
 * @param {string} htmlContent - Conteúdo HTML renderizado
 * @returns {object} { isValid: boolean, issues: string[] }
 */
export function validatePrintContent(htmlContent) {
    const issues = [];
...
export function optimizeForPrint() {
    try {
        ...
        const previewPane = document.querySelector('.pane-container.preview-pane');
        if (workspace) workspace.style.display = 'block';
        if (previewPane) previewPane.style.width = '100%';

        return true;
    } catch (e) {
        console.error('Erro ao otimizar para print:', e);
        return false;
    }
}
...
export function restoreAfterPrint() {
    try {
        ...
        console.error('Erro ao restaurar após print:', e);
        return false;
    }
}
 ...
 * @param {string} docName - Nome do documento (para título)
 * @param {Function} logger - Função de logging (opcional)
 * @returns {Promise<boolean>} Resolve quando usuário fecha diálogo
 */
export function printDocument(docName = 'document', logger = console.log) {
 ...
 * @returns {object} Estatísticas (palavras, parágrafos, imagens, etc)
 */
export function getPrintStatistics(htmlContent) {
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = htmlContent;
 ...
 */
export function generatePrintReport(docName, htmlContent) {
